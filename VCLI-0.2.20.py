...
class VCLI(object):
    ...
    def _listVmResource(self, resource, names=None):
        ...
        for _key in _objVms:
            _vm = _objVms[_key]['id']
            if _resource in ['vm-disk', 'disk']:
                _diskId = getattr(self._args, 'disk-id') if hasattr(self._args, 'disk-id') else None
                self._listVmDisk(_vm, _header, _showName, _diskId)
            elif _resource in ['vm-nic', 'nic']:
                _nicId = getattr(self._args, 'nic-id') if hasattr(self._args, 'nic-id') else None
                self._listVmNic(_vm, _header, _showName, _nicId)
            elif _resource in ['vm-snapshot', 'vm-snap', 'snapshot', 'snap']:
                self._args.snapshot = True
                self._listVmSnapshot(_vm, _header, _showName)
            _header = False
        self._print('# Total:  {0}'.format(len(_objVms)))
    ...
    def _snapshot(self, names, action='add'):
        ...
        #
        if _action in ['merge', 'remove', 'revert']:
            _match = True
            self._args.snapshot = True
